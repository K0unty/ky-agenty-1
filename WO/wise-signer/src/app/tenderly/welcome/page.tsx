...
export default function WelcomePage() {
  ...
  const handleCreateNetwork = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.tenderly.co/api/v1/account/${accountSlug.trim()}/project/${projectSlug.trim()}/vnets`,
        {
          method: "POST",
          headers: {
            "X-Access-Key": apiKey,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to create VirtualNet: ${response.status} ${
            response.statusText
      ...
      if (response.status === 409) {
        setError(
          "You already have a Wise Signer virtual network set up! Go to the virtual networks page on your Tenderly dashboard, grab the public endpoint, and add it via the 'I Already Have a Network' option."
        )
        setSetupStage("error")
        setIsLoading(false)
        return
      ...
      const publicRpc = data.rpcs?.find(
        (rpc: TenderlyRpc) => rpc.name === "Public RPC"
      )
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Failed to create the virtual network. Please check your credentials and try again."
      )
      setSetupStage("error")
    } finally {
      setIsLoading(false)
  ...
  const addNetworkToWallet = async () => {
    ...
    if (typeof window.ethereum !== "undefined") {
      ...
      setError(
        "MetaMask or blockchain wallet is not installed. Please install a blockchain wallet to continue."
      )
    }
  }
  ...
  const startChallenge = () => {
    ...
    if (networkInfo) {
      ...
      setError(
        "Network information is missing. Please set up your network first."
      )
    }
  }
  ...
  const renderContent = () => {
    ...
    if (showManualSetup) {
      return (
        <ManualSetupInstructions onBack={() => setShowManualSetup(false)} />
      )
    }
    ...
    if (networkInfo && setupStage === "initial") {
      return (
        ...
        >
          ...
          <p className="mb-6">
            We found a saved Tenderly Network configuration. Would you like to:
          </p>
    ...
    if (setupStage === "initial" && !setupOption) {
      return (
        ...
        >
          ...
          <p className="mb-8">
            To use the Ethereum transaction practice environment, you need a
            Tenderly Virtual Network. Choose one of the following options:
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            >
              ...
              <p className="text-zinc-300 text-sm mb-4">
                Set up a new Tenderly Virtual Network for practicing
                transactions.
              </p>
            </button>
          </div>
    ...
    if (setupStage === "initial" && setupOption === "existing") {
      return (
        ...
        >
          ...
          <p className="mb-6">
            Enter the RPC endpoint URL of your existing Tenderly Virtual
            Network. This will be stored locally in your browser.
          </p>
    ...
    if (setupStage === "initial" && setupOption === "create") {
      return (
        ...
        >
          ...
          <p className="mb-6">
            We'll help you set up a new Tenderly Virtual Network. You'll need a
            Tenderly account and API key to continue.
          </p>
          ...
          <div className="space-y-4 mb-6">
            <div className="mb-6 p-4 bg-zinc-700 rounded-lg">
              <div className="flex items-center mb-2">
                ...
                <h3 className="text-lg font-medium">
                  Don't want to enter your API key?
                </h3>
              </div>
              <p className="text-zinc-300 text-sm mb-3">
                You can manually set up a Tenderly Virtual Network using their
                dashboard. However, we only use the API key to generate the
                network, and it won't be stored in your browser.
              </p>
              <button
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-1">
                Your API key will only be stored in memory during setup and
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-1">
                <a
                  href="https://docs.tenderly.co/account/projects/account-project-slug"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-400 hover:text-blue-300 flex items-center gap-1 text-sm mt-2"
                >
          ...
          <div className="mb-6 p-4 bg-zinc-700 rounded">
            ...
            <div className="space-y-2 text-sm">
              ...
              {networkInfo.networkId && (
                <p>
                  <span className="text-zinc-400">Network ID:</span>{" "}
                  {networkInfo.networkId}
                </p>
              )}
            </div>
    ...
    <div className="flex flex-col items-center min-h-screen p-8 pb-20 gap-8 sm:p-20 font-[family-name:var(--font-geist-sans)] bg-gradient-to-b from-zinc-900 to-black text-white">
      <div className="text-center max-w-2xl mb-4">
        ...
        <p className="text-lg text-gray-300">
          Set up your Ethereum practice environment to safely learn transaction
          signing without risking real assets.
        </p>
      </div>
